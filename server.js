...
// ===== NIGHTLY BACKUP TO PRIVATE GITHUB REPO =====
// Bundles every file in DATA_DIR into a single JSON snapshot and uploads it
// to backups/YYYY-MM-DD.json in a private GitHub repo via the Contents API.
// Requires three env vars (set on Railway):
//   BACKUP_GITHUB_TOKEN — Personal Access Token with `repo` scope
//   BACKUP_REPO         — "owner/repo" of the private backup repo (e.g. byronmaty-arch/ads-kitchen-backups)
//   BACKUP_BRANCH       — optional, defaults to "main"
...
function buildBackupSnapshot() {
  ...
  for (const f of fs.readdirSync(DATA_DIR)) {
    ...
    try {
      ...
      snapshot.files[f] = JSON.parse(raw);
    } catch (e) {
      console.error(`[backup] failed to read ${f}:`, e.message);
    }
  }
  return snapshot;
}
...
async function runBackup() {
  ...
  // 1. Check if the file already exists today (so we can update instead of fail)
  ...
  // 2. PUT the content (create or update)
  const putPath = `/repos/${repo}/contents/${filePath}`;
  const putBody = {
  ...
  const result = await githubRequest('PUT', putPath, token, putBody);
...
// Returns ms until next 23:30 EAT
...
// Manual trigger endpoint — protected by BACKUP_GITHUB_TOKEN so only someone
// with the token can force a backup (simplest auth, no session needed).
